...
//		return new RedirectView
	...
	public String addSampleMatch(@RequestParam String startTime,@RequestParam String endTime,
			...
			@RequestParam String lastDate, Model model, HttpServletRequest request){
		...
		if(StrUtils.isNotEmptyStr(startTime)){
			...
			startTs = Timestamp.valueOf(bzStartTs);
		...
		if(StrUtils.isNotEmptyStr(endTime)){
			...
			if(endTs.getTime() < startTs.getTime()){
				String bzStartTs = TimeUtil.changeStringTimeFormat(lastDate + startTime, "yyyyMMddHHmmss", "yyyy-MM-dd HH:mm:ss");
				startTs = Timestamp.valueOf(bzStartTs);
				tsm.setStartTime(startTs.getTime()+"");
			}
		...
		if(unknownSample.equals("0")){
			if(StrUtils.isNotEmptyStr(sampleIdHide)){//已存在样本
				ts = tabSampleService.selectObjById(sampleIdHide);
				boolean needUpdTs = false;
				if(!StrUtils.isNotEmptyStr(ts.getFirstDate())) {
					ts.setFirstDate(currsampleDate);//记录首播日期
					needUpdTs = true;
				}
				if(!StrUtils.isNotEmptyStr(ts.getFirstTime())) {
					ts.setFirstTime(startTime);//记录首播时间
					needUpdTs = true;
				}
				if(!StrUtils.isNotEmptyStr(ts.getPid())) {
					ts.setPid(pidhide);
					needUpdTs = true;
				}
				if(needUpdTs == true) {
					tabSampleService.updateTabSample(ts);
				}
