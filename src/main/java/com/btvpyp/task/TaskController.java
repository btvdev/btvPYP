...
public class TaskController {
	...
	public void makeCurrentDayVideoDir() {
		//每天0点，在videos目录下创建当天日期命名的文件夹
		File dirFile;
		String currDay = TimeUtil.getTimeDir();
		String currDir = Commons.VIDEOSAVE + currDay + "/";
		try {
			dirFile = new File(currDir);
			boolean dirExist = dirFile.exists();
			if(dirExist != true) {
				dirFile.mkdir();
			}
		} catch (Exception e) {
