								...
								<tr bgcolor="#FFCC99">
									<td nowrap="nowrap">
										<c:if test="${p.needRed == 1}">
											<font color="red">${p.programName }</font>
										</c:if>
										<c:if test="${p.needRed == 0}"><font color="black">${p.programName }</font></c:if>
									</td>
...
function search(){
	$("#searchForm").attr("action", "<%=request.getContextPath()%>/tabDetail/search");
	$("#searchForm").submit();
}
function exportExcel(){
	$("#searchForm").attr("action", "<%=request.getContextPath()%>/tabDetail/exportDetailExcel");
	$("#searchForm").submit();
}
</script>
</body>
