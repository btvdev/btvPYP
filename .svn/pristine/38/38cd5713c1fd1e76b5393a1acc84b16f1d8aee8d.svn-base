...
public class TabSampleController {
	...
	public String selectSampleAdv(@RequestParam String columnType, @RequestParam String mainName,@RequestParam String secondName, 
			@RequestParam String start , @RequestParam String end,@RequestParam String brandId, @RequestParam String currPage, Model model){
		...
		if(null != brandId && !"".equals(brandId)) {
			t.setBrandId(Integer.parseInt(brandId.trim()));
		}
		int currPageInt = 1;
		...
		Page page = new Page(currPageInt, t.getPageSize(), totalCount);
...
//		model.addAttribute("totalCount", totalCount);//总条数
	...
	public String selectSampleAdvPost(@RequestParam String brandId,@RequestParam String mainName,@RequestParam String secondName,@RequestParam String columnType,
			@RequestParam String start, @RequestParam String end, @RequestParam String currPage, Model model){ 
		...
		if(null != brandId && !"".equals(brandId)){
			if(brandId.trim().equals("0")){
				t.setBrandId(-1);
			}else{
				t.setBrandId(Integer.parseInt(brandId));
			}
		}
		int currPageInt = 1;
		if(null != currPage && !"".equals(currPage)) {
...
//		model.addAttribute("totalCount", totalCount);//总条数
	...
	public String add(Model model,@RequestParam String columnTypeSearch, @RequestParam String mainNameSearch,
			...
			@RequestParam String brandIdSearch,@RequestParam String endSearch){
//		HttpSession session = request.getSession();
		...
		model.addAttribute("brandIdSearch", brandIdSearch);
	...
	public String add(TabSample tabSample,@RequestParam String columnTypeSearch, 
			...
			@RequestParam String brandIdSearch, @RequestParam String currPage, MultipartFile advFile,
		...
		model.addAttribute("advId", brandIdSearch);
...
//		}
	...
	public String update(@PathVariable String sampleId, @RequestParam String columnType,
			...
			@RequestParam String advIdSearch, @RequestParam String currPage, @RequestParam String endSearch, Model model){
...
//	@RequestMapping(value="/{sampleId}/update", method=RequestMethod.POST)
	public String update_bak(TabSample tabSample, @RequestParam String columnTypeSearch,
			@RequestParam String mainNameSearch, @RequestParam String secondNameSearch,
			@RequestParam String startSearch, @RequestParam String endSearch,MultipartFile advFile,
			@RequestParam String advIdSearch, @RequestParam String currPage, 
			HttpServletRequest request, RedirectAttributes model){
		HttpSession session = request.getSession();
		TabUser tu = (TabUser)session.getAttribute("tabUser");
	...
	@RequestMapping(value="/{sampleId}/updateWindow",method=RequestMethod.POST)
	public String updateWindow(TabSample tabSample,MultipartFile advFile,
		...
		return "sample/updateSampleSuccess";
	}
	...
	public String delete(@PathVariable String sampleId,@RequestParam String columnTypeSearch,
			...
			@RequestParam String advIdSearch , @RequestParam String currPage, HttpServletRequest request,RedirectAttributes model){
...
//						jsonContent += sampleList.get(i).getMainName() + ",";
//					}else{
//						jsonContent += sampleList.get(i).getMainName();
//					}
//				}
...
//			}
	...
	public String batchDelete(@RequestParam String columnTypeSearch, @RequestParam String ids, 
			@RequestParam String mainNameSearch,@RequestParam String secondNameSearch, @RequestParam String startSearch, 
			@RequestParam String endSearch, @RequestParam String brandIdSearch, @RequestParam String currPage, 
			HttpServletRequest request, RedirectAttributes model) {
		HttpSession session = request.getSession();
		TabUser tu = (TabUser)session.getAttribute("tabUser");
		...
		model.addAttribute("start", startSearch);
		model.addAttribute("end", endSearch);
		model.addAttribute("brandId", brandIdSearch);
		model.addAttribute("currPage", currPage);
		return "redirect:/tabSample/searchAdv";
	}
}
