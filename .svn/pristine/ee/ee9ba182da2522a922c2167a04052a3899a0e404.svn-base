		...
		<ul>
				...
				<li>
					...
					<ul>
						...
						<li style="width:230px;float:left;">
							<label>品牌号:</label><input type="text" id="brandId" name="brandId" value="${brandId }" />									
						</li>
						<li style="width:210px;float:left;">
							<label>分类:</label>
			...
			<li class="w-f-nr clearfix">
				...
				<div class="flo chech clearfix">
					<input type="checkbox" id="check_tdbrandId" checked="checked" onclick="showOrNot('check_tdbrandId')"/><label>品牌号</label>
			...
			<table>
            	<thead>
				  ...
				  <th nowrap="nowrap" name="tdbrandId">品牌号</th>
...
<!-- 				  <th nowrap="nowrap" name="tdpid">频道</th> -->
				...
				<tbody>
					...
					<tr>
						...
						<td nowrap="nowrap" name="tdbrandId">${p.brandId }</td>
    ...
    function search(){
  	...
  	function add(){
  		...
  		window.location.href="<%=request.getContextPath()%>/tabSample/add?columnTypeSearch="+columnType
  				+"&mainNameSearch="+mainName+"&secondNameSearch="+secondName+"&startSearch="+start+"&endSearch="+end+"&brandIdSearch="+brandId;
  	}
